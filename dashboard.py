...
with st.sidebar:
    ...
    if DEMO_MODE:
        ...
        tick_ms = st.select_slider(
            "Speed", options=[200, 400, 600, 1000, 2000],
            value=600, format_func=lambda x: f"{1000//x} fps"
        )
    else:
...
# ── Auto-refreshing detection fragment ───────────────────────────────────────
# st.fragment(run_every=...) re-runs only this section on the given interval
# without blocking or rerunning the whole app — works reliably on Cloud.

@st.fragment(run_every=f"{tick_ms}ms" if st.session_state.running else None)
def detection_panel():
    ...
    if not st.session_state.running:
        img_ph.info("Click ▶️ Start Detection to begin.")
        return

    # ── Generate / capture one frame ─────────────────────────────────────
    if DEMO_MODE:
        try:
        ...
        # Live mode: handled by a separate serial session (not on Cloud)
        st.warning("Live mode requires local hardware.")
        return
    ...
    # ── Render ───────────────────────────────────────────────────────────
    img_ph.image(img, caption=f"Frame #{st.session_state.frames}", use_container_width=True)
...
# ── Live mode serial loop (local only) ───────────────────────────────────────
